...
include($qa_path.'qa-include/qa-base.php');
    ...
    function qa_recent_questions(array $args = array()){
        ...
        SELECT title,postid,upvotes,acount, UNIX_TIMESTAMP(created) as created, views  FROM ".$prefix."posts 
        WHERE type= 'Q' 
        ...
        echo '<h2>Tuoreimmat kysymykset</h2>';
	...
	$i = 1;
	    ...
	    $sumvotes = $upvotes - $downvotes;
	    $acount    =   $row['acount'];
	    $views    =   $row['views'];
	    $created    =   $row['created'];
		
		$mytime = time_elapsed_string($created);
		...
		echo '<div class="qa-q-list-item">';
		echo '<div class="qa-q-item-stats">';
		echo '<span class="qa-a-count">';       
		echo '<span class="qa-a-count-data">'.$views.'</span>'; 
		echo '<span class="qa-view-count-pad">lukukertaa</span>';
		echo '</div>';
		echo '<DIV CLASS="qa-q-item-main">';
		echo '<DIV CLASS="qa-q-item-title">';                                
                echo '<a href="'.$qa_directory.'/'.qa_q_path_html($pid, $title).'">'.$title.'</a> <div class="qdate">'.$mytime.' sitten, peukutussaldo: '.$sumvotes.'</div>';       
		echo '</div>';
		echo '</div>';
		echo '</div>';
    ...
    // the function has everything include from databse to html output and can be modify as per your need.
    // to use this function please refer above usage guide.
    function qa_recent_answers(array $args = array()){
        ...
        echo '<h2>Tuoreimmat kysymykset (+ peukutussaldo)</h2>';
	echo '<div class="qa-q-list">';
	$i = 1;
        while ($row = qa_db_read_one_assoc($query, true)):
...
// the function has everything include from databse to html output and can be modify as per your need.
	...
	echo '<div class="count-item">Käyttäjiä: <div class="count-data">'.$num_rows.'</div></div>';
	...
	echo '<div class="count-item">Kysymyksiä: <div class="count-data">'.$num_rows.'</div></div>';   
	echo '<div class="clear:both">&nbsp;</div>';
	/* vastaus määrä */
	$query3 = qa_db_query_sub("select postid from qa_posts where type='A'; ") or die(mysql_error());
	$asum = 0;
	//while ($row = qa_db_read_one_assoc($query2, true)):        
	//    $acount    =   $row['acount'];
	//    $asum = $asum + $acount;
        //endwhile;
	$asum = mysql_num_rows($query3);

	echo '<div class="count-item">Vastauksia: <div class="count-data">'.$asum.'</div></div>';    
	echo '<div class="clear:both">&nbsp;</div>';
	if (isset($container))
            echo $container_close;
...
// post counts by categories    
	...
	*/
	...
	$url = "http://www.avoindata.net/questions";	
    	foreach ($arr as $key => $val)
	{
		$catname = "";
		...
		$curl = $url."/".$ntitle;
    		$rivit .= "<tr><td>".$catname."</td><td>".$perc." %</td><td>".$val."</td><td><a target='new' href='".$curl."'><img src='http://www.avoindata.net/qa-theme/tovolt/images/rss.jpg'/></a></td></tr>\n";
...
// get tags in table    
	...
	while ($row = qa_db_read_one_assoc($tags, true)):        
		...
		$ret .= "<tr><td>".$title."</td><td>".$acount."</td><td><a target='new' href='".$url."'><img src='http://www.avoindata.net/qa-theme/tovolt/images/rss.jpg'/></a></td></tr>\n";
		$i++;
        endwhile; 
	return $ret; 
    }
